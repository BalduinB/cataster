...
import {
    ...
    CommandList,
    CommandSeparator,
} from "@cataster/ui/components/base/command";
import {
    Popover,
    PopoverContent,
    PopoverTrigger,
} from "@cataster/ui/components/base/popover";
import { cn, onKeyDown } from "@cataster/ui/lib/utils";
...
type MultiSelectContextType = {
    ...
    setFreeTextValue: (value: string) => void;
};
            ...
            {[...selectedValues]
                ...
                .map((value) => (
                    ...
                    >
                        {items.get(value)}
                        {clickToRemove && (
                            <IconX className="text-muted-foreground group-hover:text-destructive size-2" />
                        )}
                    </Badge>
                ))}
            <Badge
...
export function MultiSelectContent({
    ...
    children,
    allowFreeText = false,
    freeTextPlaceholder = "Enter custom value...",
    ...props
}: {
    ...
    const { freeTextValue, setFreeTextValue, onItemAdded, toggleValue } =
        useMultiSelectContext();

    const handleAddFreeText = () => {
        const trimmedValue = freeTextValue.trim();
        if (!!trimmedValue) {
            onItemAdded(trimmedValue, trimmedValue);
            toggleValue(trimmedValue);
            setFreeTextValue("");
        } else toast.info("Du hast nichts eingegeben");
    };
    ...
    return (
        <>
            ...
            <PopoverContent className="min-w-(--radix-popover-trigger-width) p-2">
                <Command {...props}>
                    {canSearch ? (
                        <CommandInput
                            placeholder={
