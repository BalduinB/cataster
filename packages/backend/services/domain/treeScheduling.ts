...
export const validateMeasurement = (
    ...
    const comparison = options.allowZero
...
export const validateControlIntervalRRule = ({
    ...
    controlIntervalRRule: string | null;
    controlTimezone: string | null;
    baseDate: number;
}): Effect.Effect<string | null, ConflictError> => {
    const normalizedRule = normalizeOptionalString(controlIntervalRRule);
    if (!normalizedRule) return Effect.succeed(null);

    const tz = controlTimezone ?? DEFAULT_CONTROL_TIMEZONE;
    return Effect.try({
        try: () => {
            rrulestr(
                wrapRRuleLineForForcesetParse(normalizedRule, baseDate, tz),
                {
                    compatible: true,
                    forceset: true,
                    tzid: tz,
...
export const computeNextControlAt = ({
    ...
    additionalControlAt: number | null;
    baseDate: number;
    now?: number;
}): Effect.Effect<number | null, ConflictError> =>
    Effect.gen(function* () {
        ...
        if (controlIntervalRRule) {
            ...
            const nextOccurrence = yield* Effect.try({
                try: () => {
                    ...
                    new ConflictError({
                        message: "Invalid control interval RRULE",
                    }),
            });
            if (nextOccurrence) {
        ...
        if (additionalControlAt !== null && additionalControlAt >= now) {
