        ...
        <ScrollArea className="max-h-[400px]">
          {trees.length === 0 ? (
            <Empty className="py-6">
              ...
              <EmptyHeader>
                <EmptyTitle className="text-sm">Keine Bäume</EmptyTitle>
                <EmptyDescription className="text-xs">
                  Lege den ersten Baum über &quot;Neuer Baum&quot; an.
                </EmptyDescription>
              </EmptyHeader>
            </Empty>
          ) : (
...
function TreeItem({
  ...
  const vitalityColor =
    TREE_VITALITY_COLORS[tree.vitality as keyof typeof TREE_VITALITY_COLORS];
        ...
        <div className="min-w-0 flex-1">
          {tree.nextControlAt && (
            <p className="text-muted-foreground mt-2 text-xs">
              Nächste Kontrolle:{" "}
              {new Date(tree.nextControlAt).toLocaleDateString("de-DE")}
            </p>
          )}
          {tree.notes && (
