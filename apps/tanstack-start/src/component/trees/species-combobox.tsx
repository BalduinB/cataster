    ...
    <>
      <FormBase label="Baumart">
        <Combobox<string>
          ...
          itemToStringLabel={(s) =>
            species?.find((sp) => sp._id === s)?.deName ?? "Nicht gefunden"
          }
          disabled={disabled}
        >
          <ComboboxInput placeholder="Art auswählen" aria-invalid={invalid} />
          <ComboboxContent>
            <ComboboxList>
              {(species as ReadonlyArray<Species> | undefined)?.map((sp) => (
                <ComboboxItem
                  key={sp._id}
                  value={sp._id}
                  render={
                    <Item size="xs" className="p-0">
                      <ItemContent>
                        ...
                        <ItemDescription>{sp.botanicalName}</ItemDescription>
                      </ItemContent>
                    </Item>
                  }
                />
...
function AddSpeciesDialog({
  ...
  const handleSubmit = () => {
    ...
    create.mutate({ deName: trimmedDeName, botanicalName: trimmedBotanicalName });
  };
    ...
    >
      <DialogContent>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="species-botanical-name">Botanischer Name</Label>
            <Input
